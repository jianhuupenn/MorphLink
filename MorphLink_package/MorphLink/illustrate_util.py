...
def sample_illustration(f, adata, patch_info, patches, labels, plot_dir, num_cuts=5, range_step=1/4, num_sample=20, filter_mask_area=True, filter_cc_q100=False, sort_by="quantile"):
	...
	for i in range(num_cuts):
		if sort_by=="quantile":
			...
			step=(np.max(adata.obs[f])-np.min(adata.obs[f]))/num_cuts
			mx=np.min(adata.obs[f])+step*(i+1)
			mi=np.min(adata.obs[f])+step*i
		else:
			print("Wrong sort_by.")
			break
		...
		if filter_mask_area:
			...
			if m_area in sub_tmp.var.index:
				sub_tmp.obs["mask_area"]=sub_tmp.X[:, sub_tmp.var.index==m_area].flatten().tolist()
			else:
				sub_tmp.obs["mask_area"]=sub_tmp.X[:, sub_tmp.var.index=="log_"+m_area].flatten().tolist()
		...
		for j in range(len(samples)):
			...
			if f_type=="m":
				...
				sample_label=labels[channel, id, ...]
				ret_img[sample_label!=0]=np.array(color)
...
def mask_example(channel, adata, patch_info, patches, masks, plot_dir, num_samples=2, filter_mask_area=True):
	...
	ret_img=ret_img.astype(np.uint8)
	cv2.imwrite(plot_dir+'/sample_for_mask_'+str(channel)+'.png', ret_img)
	return ret_img
